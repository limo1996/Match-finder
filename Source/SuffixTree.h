//source:  http://www.geeksforgeeks.org/suffix-tree-application-5-longest-common-substring-2/
//source is modified 
...
int *rootEnd = NULL;
int *splitEnd = NULL;
int sizeOfStr= -1; //Length of input string
...
void setSuffixIndexByDFS(Node *n, int labelHeight)
{
	...
	if (leaf == 1)
	{
		...
		n->suffixIndex = sizeOfStr - labelHeight;
...
void buildSuffixTree(const char * str, int sizeOfFirst, int sizeOfWhole)
{
	text = (char *)malloc(sizeOfWhole * sizeof(char) + 1);
	strcpy(text, str);
	size1 = sizeOfFirst;
	sizeOfStr = strlen(text);
	int i;
...
char * getLongestCommonSubstr(int * length)
{
	int maxHeight = 0;
	...
	int k;
	char * res = (char *)malloc(maxHeight * sizeof(char) + 1);
	for (k = 0; k < maxHeight; k++) {
		res[k] = text[k + substringStartIndex];
	}
	res[maxHeight] = '\0';
	*length = maxHeight;
	return res;
