...
void WuManber::Initialize(const vector<const char *> &patterns,
	...
	m_vPatternMap = vector<vector<structPatternMap>>(m_nTableSize);
...
vector<char>* WuManber::Search(size_t TextLength, const char *Text, const vector<const char *> &patterns, map<const char *, char> keywords) {
			...
			while (element.end() != iter) {
				if (hash2 == (*iter).PrefixHash) {
					...
					if (0 == *ixPattern) {  // we found the end of the pattern, so match found
						result->push_back(keywords[patterns[(*iter).ix]]);
						ix += strlen(patterns[(*iter).ix]) - 1;
					}
				}
				++iter;
			}
